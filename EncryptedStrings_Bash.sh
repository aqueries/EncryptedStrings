...
function GenerateEncryptedString() {
    ...
    local ENCRYPTED=$(echo "${STRING}" | openssl enc -aes-256-cbc -md sha512 -a -S "${SALT}" -k "${K}")
...
function DecryptString() {
    ...
    echo "${1}" | openssl enc -aes-256-cbc -md sha512 -a -d -S "${2}" -k "${3}"
}
...
function DecryptString() {
    ...
    local SALT=""
    local K=""
    echo "${1}" | openssl enc -aes-256-cbc -md sha512 -a -d -S "$SALT" -k "$K"
}
